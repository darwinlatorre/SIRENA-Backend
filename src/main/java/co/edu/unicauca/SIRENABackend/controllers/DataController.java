...
/**
 * Controlador REST que maneja las operaciones relacionadas con la carga de
 * datos
 * (bookings).
 */
@RestController
    ...
    public ResponseEntity<String> loadData(@RequestBody DataReq dataRequest) {
        ...
        for (FacultyRequest faculty : dataRequest.getFacultiesList()) {
            ...
            if (buildingOptional.isPresent()) {
                ...
                return new ResponseEntity<>("Error en la facultad " + faculty.getName() + " edifico no encontrado",
        ...
        for (IncidenceReq incidence : dataRequest.getIncidencesList()) {
            if (incidenceService.saveIncidence(incidence) == null)
                return new ResponseEntity<String>("Error en la incidencia " + incidence.getName(),
